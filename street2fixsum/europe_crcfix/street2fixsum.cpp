// FSTREET2USA_Patch
// Custom checksum patcher for FIFA STREET 2 USA NDS
...
{
	printf("               FSTREET2USA_Patch\n");
	printf("FIFA STREET 2 (USA) NDS Savegame Checksum Patcher\n\n");
	...
	*/
	...
	if (crc == (uint16_t)(data[0x0A] | (data[0x0B] << 8)))
		printf("CRC Good\nPatching anyways!");
	else
		printf("CRC Bad!\nPatching...");
	...
	saveStream.seekg(0x0A);//Checksum address
